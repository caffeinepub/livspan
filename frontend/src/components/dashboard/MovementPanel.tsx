...
import { useGetMovementDay, useSaveMovementDay } from '@/hooks/useMovementQueries';
import { Activity, Zap, Loader2, Timer } from 'lucide-react';
import { toast } from 'sonner';
import { Variant_gym_run_bike_walk, Variant_intense_light_medium } from '@/backend';
...
function ActivityTypeSelector({ value, onChange, labels }: ActivityTypeSelectorProps) {
...
function IntensityToggle({ value, onChange, labels }: IntensityToggleProps) {
  ...
  const gradients: Record<Variant_intense_light_medium, string> = {
    [Variant_intense_light_medium.light]: 'from-helix-accent/30 to-helix-strand/30',
    [Variant_intense_light_medium.medium]: 'from-helix-strand/30 to-helix-glow/30',
    [Variant_intense_light_medium.intense]: 'from-helix-glow/30 to-helix-accent/50',
  };
...
function ActiveMinutesInput({ value, onChange, label }: ActiveMinutesInputProps) {
...
export default function MovementPanel() {
  ...
  const intensityLabels: Record<Variant_intense_light_medium, string> = {
    ...
    [Variant_intense_light_medium.medium]: t.movementPanel.intensityLevels.medium,
    [Variant_intense_light_medium.intense]: t.movementPanel.intensityLevels.intense,
  };
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-xs text-muted-foreground font-mono">{t.movementPanel.subtitle}</p>
        </div>
      </div>
